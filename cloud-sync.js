...
(function(){
  ...
  async function syncCustomers(d){
    await upsertRows('customers',(d.customers||[]).map(x=>({...base(x),name:x.name||'',contact:x.contact||'',address:x.address||'',phone:x.phone||'',email:x.email||'',notes:x.notes||'',customer_type:x.customerType||'auto',country_code:x.countryCode||d.settings?.countryCode||'DE',vat_id:x.vatId||'',buyer_reference:x.buyerReference||'',supplier_number:x.supplierNumber||'',e_invoice_address:x.eInvoiceAddress||'',e_invoice_required:!!x.eInvoiceRequired,electronic_invoice_consent_at:x.electronicInvoiceConsentAt||null,customer_number:x.customerNumber||'',import_source:x.importSource||'manual',import_batch_id:x.importBatchId||null,imported_at:x.importedAt||null})));
  ...
  async function pullCloud(){
    ...
    d.settings=s;
    globalThis.BusinessSetup?.applyFeatureProfile?.(s.enabledModules,s.businessMode);
    d.customers=customers.map(x=>({id:x.local_id||x.id,name:x.name,contact:x.contact,address:x.address,phone:x.phone,email:x.email,notes:x.notes,customerType:x.customer_type||'auto',countryCode:x.country_code||company.country_code||'DE',vatId:x.vat_id||'',buyerReference:x.buyer_reference||'',supplierNumber:x.supplier_number||'',eInvoiceAddress:x.e_invoice_address||'',eInvoiceRequired:!!x.e_invoice_required,electronicInvoiceConsentAt:x.electronic_invoice_consent_at||'',customerNumber:x.customer_number||'',importSource:x.import_source||'manual',importBatchId:x.import_batch_id||'',importedAt:x.imported_at||'',createdAt:x.created_at,updatedAt:x.client_updated_at||x.updated_at}));
