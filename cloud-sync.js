...
(function(){
  ...
  async function pullCloud(){
    ...
    d.settings=s;
    d.customers=customers.map(x=>({id:x.local_id||x.id,name:x.name,contact:x.contact,address:x.address,phone:x.phone,email:x.email,notes:x.notes,customerType:x.customer_type||'auto',countryCode:x.country_code||company.country_code||'DE',vatId:x.vat_id||'',buyerReference:x.buyer_reference||'',supplierNumber:x.supplier_number||'',eInvoiceAddress:x.e_invoice_address||'',eInvoiceRequired:!!x.e_invoice_required,electronicInvoiceConsentAt:x.electronic_invoice_consent_at||'',createdAt:x.created_at,updatedAt:x.client_updated_at||x.updated_at}));
