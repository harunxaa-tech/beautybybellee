/* AngebotsPilot v11.31.21 – serverseitige XRechnungs-Validierung
...
(function installServerXRechnungValidation(){
  ...
  const RUNTIME_VERSION='11.31.21';
  ...
  // v11.31.21: UBL MonetaryTotal hat eine feste Elementreihenfolge.
  ...
  function install(base){
    ...
    // v11.31.21 darf auch über 11.31.18/19 gelegt werden: die XML-Normalisierung
    ...
    if(![BASE_VERSION,'11.31.16','11.31.17','11.31.18','11.31.19'].includes(base.runtimeVersion))return false;
    ...
    function translateFixableKosit(messages){
      ...
      const errors=[];
      if(/\bBR-E-02\b|\bBR-CO-26\b|\bBR-DE-16\b/i.test(joined)){
        errors.push('Steuernummer oder USt-IdNr. des Betriebs fehlt für die XRechnung.');
      }
    ...
    function check(inv){
      // v11.31.21: wichtig – den unveränderten Basis-Check aufrufen.
    ...
    async function validatorJob(inv){
      ...
      return{
        ...
        xmlFixes:{legalMonetaryTotalOrder:'v11.31.21',inputReadiness:'tax-id-and-iban',localPreflight:'before-server',recursionFix:'v11.31.21',loaderUpgrade:'forced-current-server-layer'}
      };
    }
    ...
    window.dispatchEvent(new CustomEvent('angebotspilot:compliance-ready',{detail:{
      ...
      xmlSchemaFix:'LegalMonetaryTotal-order',inputReadiness:'tax-id-and-iban',localPreflight:'before-server',recursionFix:true
    }}));
  ...
  const timer=setInterval(()=>{
    ...
    if(tries>200){clearInterval(timer);console.error('AngebotsPilot v11.31.21 Server-Validierung konnte nicht installiert werden.')}
  },50);
