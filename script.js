...
const APP_BUILD_VERSION=globalThis.AP_BUILD_VERSION||'11.32.7';
...
function saveSettings(){
  ...
  data.settings={...data.settings,companyName:document.getElementById('companyName').value.trim(),ownerName:document.getElementById('ownerName').value.trim(),phone:document.getElementById('companyPhone').value.trim(),...addressParts,address:composeAddressParts(addressParts),weatherLocation:document.getElementById('weatherLocation').value.trim(),email:document.getElementById('companyEmail').value.trim(),countryCode,currency:profile?.currency||'EUR',appLanguage:'de',taxTreatment,tax,taxNote:document.getElementById('taxNote').value.trim()||automaticTaxNote(countryCode,taxTreatment),paymentTerm:document.getElementById('paymentTerm').value.trim(),taxNumber:document.getElementById('taxNumber').value.trim(),vatId:document.getElementById('vatId').value.trim(),iban:document.getElementById('iban').value.trim(),bankName:document.getElementById('bankName').value.trim(),offerNumberNext,invoiceNumberNext,brandAccent:validBrandHex(document.getElementById('brandAccent')?.value||data.settings.brandAccent||data.settings.brandAccentAuto||'#1f7a4d'),documentStyle:document.getElementById('documentStyle')?.value||data.settings.documentStyle||'auto',logoPosition:document.getElementById('logoPosition')?.value||data.settings.logoPosition||'left'};
  localStorage.removeItem(WEATHER_CACHE_KEY);saveData('Betrieb gespeichert','Dokumentnummern & Firmendaten aktualisiert');void persistStructuredAddressCloud('company',{...data.settings,id:globalThis.getCloudState?.()?.company?.id||''});refreshWeather(true);syncPendingBrandLogo().catch(()=>{});syncPendingBrandReference().catch(()=>{});toast('Betrieb gespeichert')
}
