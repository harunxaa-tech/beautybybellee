...
const APP_BUILD_VERSION='11.30.0';
...
function newInvoice(){
  ...
  invoiceDraftLines=[{name:'',qty:1,unit:'Pauschale',price:0}];renderInvoiceLines(false);applyInvoiceLockState(null);populateInvoiceTaxControls({countryCode:currentCountryCode(),taxTreatment:data.settings.taxTreatment||'standard',tax:Number(data.settings.tax)||0,taxNote:data.settings.taxNote||''});showScreen('invoiceEditor')
}
...
async function saveInvoice(){
  ...
  const shouldFinalize=['open','paid'].includes(inv.status);let compliance={ok:true,route:{mode:'pdf',format:'pdf'}};if(shouldFinalize){compliance=await runInvoiceComplianceBeforeFinalize(inv);if(!compliance.ok){document.getElementById('invoiceStatus').value='draft';inv.status='draft';return}const ok=await appConfirm({title:'Rechnung ausstellen?',text:`${inv.number} wird danach gegen inhaltliche Änderungen gesperrt. Falls später etwas geändert werden muss, erstellst du eine Korrektur oder ein Storno.`,confirmLabel:'Jetzt ausstellen',icon:'🔒'});if(!ok){document.getElementById('invoiceStatus').value='draft';inv.status='draft';return}}
...
async function finalizeInvoiceById(id){const inv=data.invoices.find(x=>x.id===id);if(!inv)return;if(isInvoiceLocked(inv))return editInvoice(id);invoiceCalc(inv);const compliance=await runInvoiceComplianceBeforeFinalize(inv);if(!compliance.ok)return;const ok=await appConfirm({title:'Rechnung ausstellen?',text:`${inv.number} wird finalisiert und danach gegen Änderungen gesperrt.${compliance.route?.mode==='structured'?' Zusätzlich wird das strukturierte E‑Rechnungs-Original unverändert archiviert.':''}`,confirmLabel:'Rechnung ausstellen',icon:'🔒'});if(!ok)return;const archived=await archiveInvoiceStructuredOriginal(inv,compliance);if(!archived)return;finalizeInvoiceRecord(inv,'open');saveData('Rechnung finalisiert',inv.number);toast(compliance.route?.mode==='structured'?'✅ E‑Rechnung ausgestellt':'🔒 Rechnung ausgestellt')}
