...
function newCustomer(){
  ['customerId','custName','custContact','custAddress','custPhone','custEmail','custNotes','custVatId','custBuyerReference','custSupplierNumber','custEInvoiceAddress'].forEach(id=>{const el=document.getElementById(id);if(el)el.value=''});
...
function editCustomer(id){
  ...
  const values={customerId:c.id,custName:c.name,custContact:c.contact||'',custAddress:c.address||'',custPhone:c.phone||'',custEmail:c.email||'',custNotes:c.notes||'',custVatId:c.vatId||'',custBuyerReference:c.buyerReference||'',custSupplierNumber:c.supplierNumber||'',custEInvoiceAddress:c.eInvoiceAddress||''};
...
function saveCustomer(){
  const id=document.getElementById('customerId').value,old=id?data.customers.find(x=>x.id===id):null,obj={...(old||{}),id:id||uid(),name:document.getElementById('custName').value.trim(),contact:document.getElementById('custContact').value.trim(),address:document.getElementById('custAddress').value.trim(),phone:document.getElementById('custPhone').value.trim(),email:document.getElementById('custEmail').value.trim(),notes:document.getElementById('custNotes').value.trim(),customerType:document.getElementById('custCustomerType')?.value||'auto',countryCode:document.getElementById('custCountryCode')?.value||currentCountryCode(),vatId:document.getElementById('custVatId')?.value.trim()||'',buyerReference:document.getElementById('custBuyerReference')?.value.trim()||'',supplierNumber:document.getElementById('custSupplierNumber')?.value.trim()||'',eInvoiceAddress:document.getElementById('custEInvoiceAddress')?.value.trim()||'',eInvoiceRequired:!!document.getElementById('custEInvoiceRequired')?.checked};
  if(!obj.name)return toast('Name fehlt');if(id)data.customers[data.customers.findIndex(x=>x.id===id)]=obj;else data.customers.push(obj);saveData();showScreen('customers');toast('Kunde gespeichert')
}
...
function renderOfferEditorSummary(){
  ...
  const ui=offerTaxUX(),profile=currentCountryProfile(),taxAmount=Math.max(0,(Number(o.total)||0)-(Number(o.subtotal)||0)),count=(draftLines||[]).length,positionMeta=document.getElementById('offerPositionMeta'),badge=document.getElementById('offerSummaryTaxBadge'),note=document.getElementById('offerSummaryTaxNote');
  ...
  if(offerHasDisplayedTax()){
    rows.push(`<div><span>Zwischensumme netto</span><b>${money(o.subtotal||0)}</b></div>`);
...
function saveSettings(){
  ...
  localStorage.removeItem(WEATHER_CACHE_KEY);saveData('Betrieb gespeichert','Dokumentnummern & Firmendaten aktualisiert');refreshWeather(true);syncPendingBrandLogo().catch(()=>{});syncPendingBrandReference().catch(()=>{});toast('Betrieb gespeichert')
}
