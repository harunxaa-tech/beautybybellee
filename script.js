...
async function listCustomerFiles(customerId){
  ...
  return new Promise((resolve,reject)=>{
    ...
    req.onsuccess=()=>{db.close();const rows=req.result||[];rows.sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));resolve(rows)};
    req.onerror=()=>reject(req.error);
  });
}
...
function paperHTML(o){
  ...
  const travelRow=o.travel?`<tr><td class="posCol">${String(o.lines.filter(l=>l.name).length+1).padStart(2,'0')}</td><td><b>Anfahrt</b></td><td class="num">1 Pauschale</td><td class="num">${euro(o.travel)}</td><td class="num strong">${euro(o.travel)}</td></tr>`:'';
  ...
  return `<div class="offerPaper professionalPaper">
    ...
    <table class="paperTable proTable"><thead><tr><th>Pos.</th><th>Beschreibung</th><th class="num">Menge</th><th class="num">Einzelpreis</th><th class="num">Gesamt</th></tr></thead><tbody>${lineRows}${travelRow}</tbody></table>
    <div class="proSummary">
      ...
      <div class="totals proTotals"><div><span>Zwischensumme</span><b>${euro(o.subtotal)}</b></div>${o.tax?`<div><span>Umsatzsteuer ${o.tax}%</span><b>${euro(o.subtotal*o.tax/100)}</b></div>`:''}<div class="grand"><span>Gesamtbetrag</span><strong>${euro(o.total)}</strong></div></div>
    </div>
...
function findInvoiceForOffer(offerOrId){
  ...
  const byDirect=(data.invoices||[]).find(inv=>inv.offerId===offerId);
  if(byDirect)return byDirect;

  if(offerNumber){
    const byOfferNumber=(data.invoices||[]).find(inv=>
      String(inv.offerNumber||'').trim()===String(offerNumber).trim() ||
      String(inv.sourceOfferNumber||'').trim()===String(offerNumber).trim()
    );
    if(byOfferNumber)return byOfferNumber;
  }

  const job=(data.jobs||[]).find(j=>j.offerId===offerId&&j.invoiceId);
  if(job){
    const byJob=data.invoices.find(inv=>inv.id===job.invoiceId);
    if(byJob)return byJob;
  }
  return null;
}

function invoiceNumberExists(number,excludeId=''){
  const n=String(number||'').trim().toLowerCase();
  if(!n)return false;
  return (data.invoices||[]).some(inv=>inv.id!==excludeId&&String(inv.number||'').trim().toLowerCase()===n);
}

function nextUniqueInvoiceNumber(){
  let candidate=nextInvoiceNumber();
  if(!invoiceNumberExists(candidate))return candidate;

  let guard=0;
  while(invoiceNumberExists(candidate) && guard<500){
    guard++;
    const m=String(candidate).match(/^(.*?)(\d+)$/);
    if(m){
      const prefix=m[1],num=parseInt(m[2],10)+1;
      candidate=prefix+String(num).padStart(m[2].length,'0');
    }else{
      candidate=String(candidate)+'-'+guard;
    }
  }
  return candidate;
}
function createInvoiceFromOfferObject(offer){
  ...
  const existing=findInvoiceForOffer(offer);
  if(existing)return existing;

  const linkedJob=(data.jobs||[]).find(j=>j.offerId===offer.id);
  const date=todayISO();
  const inv={
    id:uid(),
    number:nextUniqueInvoiceNumber(),
    customerId:offer.customerId,
    date,
    dueDate:addDaysISO(date,paymentDays()),
    status:'draft',
    subject:offer.subject||linkedJob?.title||'Ausgeführte Arbeiten',
    notes:'Aus dem abgeschlossenen Angebot erstellt. Bitte tatsächliche Leistungen, Mengen und Zusatzarbeiten vor Versand prüfen.',
    lines:structuredClone(offer.lines||[]),
    travel:Number(offer.travel)||0,
    discount:Number(offer.discount)||0,
    tax:Number(offer.tax ?? data.settings.tax)||0,
    offerId:offer.id,
  ...
  if(linkedJob){
    linkedJob.invoiceId=inv.id;
    if(linkedJob.status!=='done')linkedJob.status='done';
  }

  addAudit('Rechnungsentwurf aus Angebot erstellt',`${inv.number} · ${offer.number||offer.subject||''}`);
  return inv;
}
...
function saveInvoice(){const inv=invoiceObject()
  if(invoiceNumberExists(inv.number,inv.id)){
    toast('⚠️ Diese Rechnungsnummer existiert bereits');
    return;
  }
  if(inv.offerId){
    const linkedOffer=(data.offers||[]).find(o=>o.id===inv.offerId);
    const duplicate=findInvoiceForOffer(linkedOffer||inv.offerId);
    if(duplicate && duplicate.id!==inv.id){
      toast(`⚠️ Für dieses Angebot existiert bereits Rechnung ${duplicate.number}`);
      editInvoice(duplicate.id);
      return;
    }
  }
;if(!inv.customerId)return toast('Kunde auswählen');if(!inv.subject)return toast('Betreff fehlt');if(!inv.lines.some(l=>l.name))return toast('Position fehlt');const i=data.invoices.findIndex(x=>x.id===inv.id);if(i>=0)data.invoices[i]=inv;else data.invoices.push(inv);saveData('Rechnung gespeichert',inv.number);showScreen('invoices');toast('Rechnung gespeichert')}
