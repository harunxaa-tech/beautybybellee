/* AngebotsPilot v11.31.25 – serverseitige XRechnungs-Validierung
...
(function installServerXRechnungValidation(){
  ...
  const RUNTIME_VERSION='11.31.25';
  ...
  // v11.31.25: UBL MonetaryTotal hat eine feste Elementreihenfolge.
  ...
  function install(base){
    ...
    // v11.31.25 darf auch über 11.31.18/19 gelegt werden: die XML-Normalisierung
    ...
    if(![BASE_VERSION,'11.31.16','11.31.17','11.31.18','11.31.19','11.31.20','11.31.21','11.31.22','11.31.23','11.31.24'].includes(base.runtimeVersion))return false;
    ...
    function check(inv){
      // v11.31.25: wichtig – den unveränderten Basis-Check aufrufen.
    ...
    async function validatorJob(inv){
      ...
      return{
        ...
        xmlFixes:{legalMonetaryTotalOrder:'v11.31.25',inputReadiness:'tax-id-and-iban',sellerIdentifier:'BT-29-normalized-after-EndpointID',loaderFix:'current-layer-v11.31.25',localPreflight:'before-server',recursionFix:'v11.31.25',loaderUpgrade:'forced-current-server-layer'}
      };
    }
  ...
  globalThis.APComplianceUpgradeTo113125=install;

  let tries=0;
  const timer=setInterval(()=>{
    tries++;
    if(install(globalThis.APCompliance)){clearInterval(timer);return}
    if(tries>200){clearInterval(timer);console.error('AngebotsPilot v11.31.25 Server-Validierung konnte nicht installiert werden.')}
  },50);
