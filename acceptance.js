/* AngebotsPilot v11.9 – Vor-Ort-Abnahme & Kundenunterschrift */
(function(){
  'use strict';
  ...
  function operatorName(){const {membership,session}=cloud();return membership?.display_name||membership?.email||session?.user?.user_metadata?.full_name||session?.user?.email||'';}
  ...
  function fitCanvas(){
    const canvas=q('acceptanceSignature'); if(!canvas)return;
    const rect=canvas.getBoundingClientRect();
    const ratio=Math.max(1,Math.min(3,window.devicePixelRatio||1));
    const cssW=Math.max(280,Math.floor(rect.width||520));
    const cssH=170;
    const old=dirty?canvas.toDataURL('image/png'):null;
    canvas.width=Math.floor(cssW*ratio); canvas.height=Math.floor(cssH*ratio);
    canvas.style.height=cssH+'px';
    const ctx=canvas.getContext('2d');ctx.setTransform(ratio,0,0,ratio,0,0);ctx.lineCap='round';ctx.lineJoin='round';ctx.lineWidth=2.4;ctx.strokeStyle='#f4f6fb';
    if(old){const img=new Image();img.onload=()=>ctx.drawImage(img,0,0,cssW,cssH);img.src=old;}
  }
  function point(ev){const c=q('acceptanceSignature'),r=c.getBoundingClientRect(),e=ev.touches?.[0]||ev.changedTouches?.[0]||ev;return{x:e.clientX-r.left,y:e.clientY-r.top};}
  function startDraw(ev){ev.preventDefault();drawing=true;last=point(ev);}
  function moveDraw(ev){if(!drawing)return;ev.preventDefault();const p=point(ev),ctx=q('acceptanceSignature').getContext('2d');ctx.beginPath();ctx.moveTo(last.x,last.y);ctx.lineTo(p.x,p.y);ctx.stroke();last=p;dirty=true;}
  function stopDraw(ev){if(drawing)ev?.preventDefault?.();drawing=false;}
  function clearSignature(){const c=q('acceptanceSignature');if(c)c.getContext('2d').clearRect(0,0,c.width,c.height);dirty=false;}

  function attachCanvas(){
    const c=q('acceptanceSignature');if(!c||c.dataset.bound)return;c.dataset.bound='1';
    c.addEventListener('pointerdown',startDraw);c.addEventListener('pointermove',moveDraw);c.addEventListener('pointerup',stopDraw);c.addEventListener('pointercancel',stopDraw);c.addEventListener('pointerleave',stopDraw);
    c.addEventListener('touchstart',startDraw,{passive:false});c.addEventListener('touchmove',moveDraw,{passive:false});c.addEventListener('touchend',stopDraw,{passive:false});
  }
  ...
  function begin(job,oldJob){
    ...
    const customer=linkedCustomer(job),offer=linkedOffer(job);
  ...
  function buildPdf(snapshot,signatureBytes){
    ...
    wrap('Erklärung: Die oben beschriebenen Leistungen wurden gemeinsam besichtigt. Der Kunde bestätigt mit seiner Unterschrift den ausgewählten Abnahmestatus und die dokumentierten Vorbehalte bzw. Mängel. Gesetzliche Rechte bleiben unberührt.',76).forEach(x=>add(x,false,8));add('');
    add(`Unterschreibende Person: ${snapshot.customerSigner}`,true,10);add(`Dokumentiert durch: ${snapshot.operatorName||'Betrieb / Mitarbeiter'}`,false,9);add('Unterschrift Kunde:',true,10);

    let y=800,content='';for(const l of lines){if(!l.txt){y-=8;continue;}if(y<245)break;content+=`BT /${l.bold?'F2':'F1'} ${l.sz} Tf 54 ${y.toFixed(1)} Td (${pdfEscape(l.txt)}) Tj ET\n`;y-=l.sz+5;}
    const dims=jpegDimensions(signatureBytes),maxW=330,maxH=95,scale=Math.min(maxW/dims.w,maxH/dims.h),w=dims.w*scale,h=dims.h*scale;
    content+=`q ${w.toFixed(2)} 0 0 ${h.toFixed(2)} 54 108 cm /Im1 Do Q\n`;
    content+=`0.8 w 54 100 m 384 100 l S\nBT /F1 7 Tf 54 82 Td (Dokument-ID: ${pdfEscape(snapshot.documentId)}) Tj ET\n`;
    const contentBytes=enc.encode(content);
    push('%PDF-1.4\n%AP11\n');
    addObj(1,'<< /Type /Catalog /Pages 2 0 R >>');
    addObj(2,'<< /Type /Pages /Kids [3 0 R] /Count 1 >>');
    addObj(3,'<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 4 0 R /F2 5 0 R >> /XObject << /Im1 7 0 R >> >> /Contents 6 0 R >>');
    ...
    addObj(7,`<< /Type /XObject /Subtype /Image /Width ${dims.w} /Height ${dims.h} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${signatureBytes.length} >>\nstream\n`,signatureBytes,'\nendstream');
  ...
  async function finalize(){
    ...
    if(!dirty)return globalThis.toast?.('Bitte den Kunden unterschreiben lassen.');
    ...
    try{
      ...
      const sigUrl=q('acceptanceSignature').toDataURL('image/jpeg',0.92),sigBytes=dataUrlBytes(sigUrl),sigHash=await sha256(sigBytes);
      const pdf=buildPdf(snap,sigBytes),pdfHash=await sha256(pdf);
      ...
      snap.pdfStoragePath=uploaded.storagePath;snap.pdfSha256=pdfHash;snap.signatureSha256=sigHash;
      const snapHash=await sha256(new TextEncoder().encode(JSON.stringify(snap)));
      const {error}=await client.from('job_acceptances').insert({company_id:company.id,job_id:cj.id,customer_id:cj.customer_id,protocol_number:documentId,acceptance_result:snap.acceptanceStatus==='accepted_clean'?'accepted':'accepted_with_reservations',customer_name:snap.customerSigner,defects_text:snap.defects,remaining_work:snap.acceptanceStatus==='accepted_with_reservations'?snap.defects:'',notes:snap.notes,signature_data:sigUrl,snapshot:snap,snapshot_hash:snapHash,created_by:session.user.id,signed_at:signedAt});
      if(error){try{await globalThis.CloudFiles?.deleteCloudFile?.(uploaded.id)}catch(ignore){}throw error;}
      q('acceptanceBackdrop').classList.add('hidden');document.body.classList.remove('acceptanceOpen');
      bypass=true;pending=null;
      await globalThis.Notifications?.notifyOwnerOffice?.('Baustelle abgenommen',`${job.title} · ${snap.customerSigner} hat das Abnahmeprotokoll unterschrieben.`,{type:'acceptance',tag:`acceptance-${job.id}-${documentId}`,url:'./?screen=jobs',metadata:{screen:'jobs',job_local_id:job.id}}).catch(()=>{});
      globalThis.toast?.('✓ Abnahme unterschrieben und sicher gespeichert');
      setTimeout(()=>globalThis.saveJob?.(),250);
    }catch(e){console.error('Abnahme',e);globalThis.toast?.('Abnahme konnte nicht sicher gespeichert werden. Bitte erneut versuchen.');}
    finally{btn.disabled=false;btn.textContent='✓ Unterschreiben & Auftrag abschließen';}
  }
  ...
  globalThis.Acceptance={begin,cancel,finalize,setStatus,clearSignature,shouldIntercept,consumeBypass};
  globalThis.setAcceptanceStatus=setStatus;globalThis.clearAcceptanceSignature=clearSignature;globalThis.finalizeAcceptance=finalize;globalThis.cancelAcceptance=cancel;
})();
