...
(function(){
  ...
  async function createCompanyFromPending(){
    const p=pendingData()||{}, l=localSettings();
  ...
  function fillRegistrationDefaults(){
    const s=localSettings(),p=pendingData()||{};
    signupDraft={...p};
    if(q('entryRegisterName'))q('entryRegisterName').value=p.name||s.ownerName||'';
    if(q('entryRegisterEmail'))q('entryRegisterEmail').value=p.email||'';
    if(q('entryRegisterCompany'))q('entryRegisterCompany').value=p.companyName||s.companyName||'';
    if(q('entryRegisterTrade'))q('entryRegisterTrade').value=p.trade||s.trade||'garden';
    if(q('entryRegisterAddress'))q('entryRegisterAddress').value=p.address||s.address||'';
    if(q('entryRegisterTax'))q('entryRegisterTax').value=String(p.tax??s.tax??0);
  }

  function fillRecoveryDefaults(){
    const p=pendingData()||{},s=localSettings();
