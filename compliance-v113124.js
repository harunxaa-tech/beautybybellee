/* AngebotsPilot v11.31.24 – serverseitige XRechnungs-Validierung
...
(function installServerXRechnungValidation(){
  ...
  const RUNTIME_VERSION='11.31.24';
  ...
  // v11.31.24: UBL MonetaryTotal hat eine feste Elementreihenfolge.
  ...
  function ensureSellerIdentifier(xml){
    ...
    // UBL Party hat eine feste Reihenfolge: EndpointID (falls vorhanden) MUSS vor
    // PartyIdentification stehen. v11.31.23 setzte BT-29 direkt nach <cac:Party>
    // und damit vor EndpointID; KoSIT/XSD lehnte das korrekt ab.
    const insertionAnchor=block.match(/<cac:Party>(?:\s*<cbc:EndpointID\b[^>]*>[\s\S]*?<\/cbc:EndpointID>)?/);
    if(!insertionAnchor)return source;
    const patched=block.replace(insertionAnchor[0],insertionAnchor[0]+identification);
  ...
  function install(base){
    ...
    // v11.31.24 darf auch über 11.31.18/19 gelegt werden: die XML-Normalisierung
    ...
    if(![BASE_VERSION,'11.31.16','11.31.17','11.31.18','11.31.19','11.31.20','11.31.21','11.31.22'].includes(base.runtimeVersion))return false;
    ...
    function check(inv){
      // v11.31.24: wichtig – den unveränderten Basis-Check aufrufen.
    ...
    async function validatorJob(inv){
      ...
      return{
        ...
        xmlFixes:{legalMonetaryTotalOrder:'v11.31.24',inputReadiness:'tax-id-and-iban',sellerIdentifier:'BT-29-after-EndpointID',loaderFix:'current-layer-v11.31.24',localPreflight:'before-server',recursionFix:'v11.31.24',loaderUpgrade:'forced-current-server-layer'}
      };
    }
    ...
    window.dispatchEvent(new CustomEvent('angebotspilot:compliance-ready',{detail:{
      ...
      xmlSchemaFix:'LegalMonetaryTotal-order',sellerIdentifier:'BT-29-after-EndpointID',inputReadiness:'tax-id-and-iban',localPreflight:'before-server',recursionFix:true
    }}));
  ...
  const timer=setInterval(()=>{
    ...
    if(tries>200){clearInterval(timer);console.error('AngebotsPilot v11.31.24 Server-Validierung konnte nicht installiert werden.')}
  },50);
