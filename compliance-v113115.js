/* AngebotsPilot v11.31.15 – serverseitige XRechnungs-Validierung
...
(function installServerXRechnungValidation(){
  ...
  const RUNTIME_VERSION='11.31.15';
    ...
    try{
      ...
      if(error){
        ...
        return{
          ok:true,
          transport:'error',
          server_preflight:{status:'unavailable',valid:null,messages:[{severity:'warning',message:'Serverprüfung konnte gerade nicht erreicht werden. Der lokale Vorabcheck bleibt aktiv.'}]},
          official_kosit:{status:'not-run',valid:null,required:false}
        };
      }
  ...
  function install(base){
    ...
    async function prepareForFinalization(inv){
      ...
      const officialFailed=official.status==='failed'||(official.required===true&&official.status==='error');
  ...
  const timer=setInterval(()=>{
    ...
    if(tries>200){clearInterval(timer);console.error('AngebotsPilot v11.31.15 Server-Validierung konnte nicht installiert werden.')}
  },50);
