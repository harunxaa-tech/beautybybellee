...
(function(){
  ...
  function diagnostics(data){
    ...
    return{ok:issues.length===0,schemaVersion:SCHEMA_VERSION,storageMode:'local',cloudReady:true,companyId:data?.meta?.companyId||'',userId:data?.meta?.currentUserId||'',revision:data?.meta?.localRevision||0,issues};
  }
