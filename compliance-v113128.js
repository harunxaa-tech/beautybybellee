/* AngebotsPilot v11.31.28 – serverseitige XRechnungs-Validierung
...
(function installServerXRechnungValidation(){
  ...
  const RUNTIME_VERSION='11.31.28';
  ...
  // v11.31.28: UBL MonetaryTotal hat eine feste Elementreihenfolge.
  ...
  // v11.31.28: XRechnung verlangt auch bei Gutschrift/Storno Zahlungsanweisungen (BG-16).
  // Der Basisgenerator ließ PaymentMeans bei CreditNote absichtlich weg. KoSIT meldete
  // daraufhin BR-DE-1. Wir ergänzen die bereits hinterlegte IBAN ohne Beträge oder
  // Beleginhalt zu verändern. Die UBL-Reihenfolge bleibt: Delivery -> PaymentMeans ->
  // AllowanceCharge/TaxTotal.
  function ensurePaymentInstructions(xml,inv){
    let source=String(xml||'');
    if(/<cac:PaymentMeans\b/.test(source))return source;
    const s=globalThis.data?.settings||{};
    const iban=String(s.iban||'').replace(/\s+/g,'').toUpperCase();
    if(!iban)return source;
    const company=String(s.companyName||'').trim();
    const payment=`<cac:PaymentMeans><cbc:PaymentMeansCode>58</cbc:PaymentMeansCode><cbc:PaymentID>${xmlEscape(inv?.number||'')}</cbc:PaymentID><cac:PayeeFinancialAccount><cbc:ID>${xmlEscape(iban)}</cbc:ID>${company?`<cbc:Name>${xmlEscape(company)}</cbc:Name>`:''}</cac:PayeeFinancialAccount></cac:PaymentMeans>`;
    const anchor=source.match(/<cac:AllowanceCharge\b/)?.[0]||source.match(/<cac:TaxTotal\b/)?.[0]||source.match(/<cac:LegalMonetaryTotal\b/)?.[0]||'';
    if(!anchor)return source;
    return source.replace(anchor,payment+anchor);
  }

  function assertPaymentInstructions(xml){
    if(!/<cac:PaymentMeans\b[\s\S]*?<cbc:PaymentMeansCode(?:\s[^>]*)?>58<\/cbc:PaymentMeansCode>[\s\S]*?<\/cac:PaymentMeans>/.test(String(xml||''))){
      throw new Error('XRechnung-Zahlungsanweisung (BG-16) konnte technisch nicht erzeugt werden.');
    }
  }
  ...
  function ensureSellerIdentifier(xml){
    ...
    const supplierBlock=supplier[0];
    const partyMatch=supplierBlock.match(/<cac:Party>([\s\S]*?)<\/cac:Party>/);
    if(!partyMatch)return source;

    const s=globalThis.data?.settings||{};
    const sellerId=String(s.vatId||s.taxNumber||'').trim();

    // Upgrade-sicher: ältere Runtime-Schichten können PartyIdentification bereits
    // an der falschen UBL-Position eingefügt haben. Vorhandene Kennungen werden
    // deshalb herausgenommen, dedupliziert und in korrekter Reihenfolge neu gesetzt.
    let inner=partyMatch[1];
    const idPattern=/<cac:PartyIdentification>[\s\S]*?<\/cac:PartyIdentification>/g;
    const existing=[...inner.matchAll(idPattern)].map(m=>m[0]);
    inner=inner.replace(idPattern,'');

    const ids=[];
    const seen=new Set();
    for(const node of existing){
      const key=String(node||'').replace(/\s+/g,'');
      if(key&&!seen.has(key)){seen.add(key);ids.push(node)}
    }

    if(!ids.length&&sellerId){
      ids.push(`<cac:PartyIdentification><cbc:ID>${xmlEscape(sellerId)}</cbc:ID></cac:PartyIdentification>`);
    }
    if(!ids.length)return source;

    const identification=ids.join('');
    const endpoint=inner.match(/<cbc:EndpointID\b[^>]*>[\s\S]*?<\/cbc:EndpointID>/);
    if(endpoint){
      inner=inner.replace(endpoint[0],endpoint[0]+identification);
    }else{
      inner=identification+inner;
    }

    const correctedParty=`<cac:Party>${inner}</cac:Party>`;
    const correctedSupplier=supplierBlock.replace(partyMatch[0],correctedParty);
    return source.replace(supplierBlock,correctedSupplier);
  }

  function assertSellerPartyOrder(xml){
    const supplier=String(xml||'').match(/<cac:AccountingSupplierParty>([\s\S]*?)<\/cac:AccountingSupplierParty>/)?.[0]||'';
    if(!supplier)return;
    const endpointIndex=supplier.indexOf('<cbc:EndpointID');
    const identifierIndex=supplier.indexOf('<cac:PartyIdentification>');
    if(endpointIndex>=0&&identifierIndex>=0&&identifierIndex<endpointIndex){
      throw new Error('XRechnung-Verkäuferkennung (BT-29) steht technisch an der falschen UBL-Position.');
    }
  }
  ...
  function install(base){
    ...
    // v11.31.28 darf auch über 11.31.18/19 gelegt werden: die XML-Normalisierung
    ...
    if(![BASE_VERSION,'11.31.16','11.31.17','11.31.18','11.31.19','11.31.20','11.31.21','11.31.22','11.31.23','11.31.24','11.31.25','11.31.26','11.31.27'].includes(base.runtimeVersion))return false;
    ...
    function generateUbl(inv,format='xrechnung'){
      ...
      if(format==='xrechnung'){
        xml=ensureSellerIdentifier(xml);
        xml=ensurePaymentInstructions(xml,inv);
        const s=globalThis.data?.settings||{};
        const hasSourceId=!!String(s.vatId||s.taxNumber||'').trim();
        const supplier=String(xml||'').match(/<cac:AccountingSupplierParty>([\s\S]*?)<\/cac:AccountingSupplierParty>/)?.[0]||'';
        if(hasSourceId&&!/<cac:PartyIdentification>[\s\S]*?<cbc:ID(?:\s[^>]*)?>[^<]+<\/cbc:ID>[\s\S]*?<\/cac:PartyIdentification>/.test(supplier)){
          throw new Error('XRechnung-Verkäuferkennung (BT-29) konnte technisch nicht erzeugt werden.');
        }
        assertSellerPartyOrder(xml);
        assertPaymentInstructions(xml);
      }
      return xml;
    }
    ...
    function inputReadiness(inv){
      ...
      if(!payment58){
        errors.push('Zahlungsweg/IBAN fehlt für die XRechnung.');
      }else if(!ibanIsValid(s.iban,sellerCountry)){
    ...
    function translateFixableKosit(messages){
      ...
      if(/\bBR-DE-1\b|PAYMENT INSTRUCTIONS|\bBG-16\b/i.test(joined)){
        errors.push('Zahlungsanweisung/IBAN fehlt für die XRechnung.');
      }
    ...
    function check(inv){
      // v11.31.28: wichtig – den unveränderten Basis-Check aufrufen.
    ...
    async function validatorJob(inv){
      ...
      return{
        ...
        xmlFixes:{legalMonetaryTotalOrder:'v11.31.28',paymentInstructions:'BG-16-creditnote-and-invoice',inputReadiness:'tax-id-iban-and-payment',sellerIdentifier:'BT-29-normalized-after-EndpointID',loaderFix:'current-layer-v11.31.28',localPreflight:'before-server',recursionFix:'v11.31.28',loaderUpgrade:'forced-current-server-layer'}
      };
    }
    ...
    async function prepareForFinalization(inv){
      ...
      try{
        ...
        inv.complianceReport={
          ...(inv.complianceReport||{}),
          structuredXmlSha256:job?.xmlSha256||'',
          validatorJob:{
      ...
      // Erst die exakt zu archivierende XML-Fassung serverseitig/mit KoSIT prüfen.
      // Eine abgelehnte XML darf niemals als unverändertes Original im Firmenarchiv landen.
      const server=await serverValidate(inv,job);
      const validated=applyServerValidation(inv,{...local,archived:null,validatorJob:job},server);
      ...
      try{
        ...
        const message='E-Rechnungs-Original konnte nach bestandener Prüfung nicht sicher archiviert werden: '+String(error?.message||error);
        const errors=uniq([...(validated.errors||[]),message]);
        inv.complianceStatus='blocked';
        inv.complianceReport={...(inv.complianceReport||{}),errors};
        return{...validated,ok:false,status:'blocked',failureKind:'archive',errors,archived:null};
      }
      ...
      inv.complianceReport={
        ...
        validatorJob:{
          ...(inv.complianceReport?.validatorJob||{}),
          protocol:job?.protocol,filename:job?.filename,xmlSha256:job?.xmlSha256,
          validator:job?.validator,officialStatus:'passed',xmlFixes:job?.xmlFixes
        }
      };
      return{...validated,archived,validatorJob:job};
    }
    ...
    window.dispatchEvent(new CustomEvent('angebotspilot:compliance-ready',{detail:{
      ...
      xmlSchemaFix:'LegalMonetaryTotal-order',sellerIdentifier:'BT-29-normalized-after-EndpointID',inputReadiness:'tax-id-iban-and-payment',localPreflight:'before-server',recursionFix:true,paymentInstructions:'BG-16-creditnote-and-invoice',archiveOrder:'validate-before-archive'
    }}));
    try{globalThis.refreshInvoiceComplianceUI?.()}catch(e){}
    return true;
  }

  // cloud-config kann die Installation erneut anstoßen, falls diese Datei schon
  // geladen war, während noch eine ältere Compliance-Runtime aktiv war.
  globalThis.APComplianceUpgradeTo113128=install;

  let tries=0;
  const timer=setInterval(()=>{
    tries++;
    if(install(globalThis.APCompliance)){clearInterval(timer);return}
    if(tries>200){clearInterval(timer);console.error('AngebotsPilot v11.31.28 Server-Validierung konnte nicht installiert werden.')}
  },50);
